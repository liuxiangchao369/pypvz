...
class Level(tool.State):
    ...
    def refreshWaves(self, current_time, survival_rounds=0):
                            ...
                            for map_y in range(0, 4):
                                for map_x in range(4, 8):
                                    ...
                                    if ((not self.map.map[map_y][map_x][c.MAP_PLANT]) or
                                            (all((i in exception_objects) for i in
        ...
        if (self.wave_num % 10 != 9):
            if ((current_time - self.wave_time >= 25000 + random.randint(0, 6000)) or (
                    self.bar_type == c.CHOOSEBAR_BOWLING and current_time - self.wave_time >= 12500 + random.randint(0,
                                                                                                                     3000))):
                self.wave_num += 1
                self.wave_time = current_time
                self.wave_zombies = self.waves[self.wave_num - 1]
            ...
            if ((current_time - self.wave_time >= 45000) or (
                    self.bar_type != c.CHOOSEBAR_STATIC and current_time - self.wave_time >= 25000)):
                self.wave_num += 1
                self.wave_time = current_time
                self.wave_zombies = self.waves[self.wave_num - 1]
                self.zombie_num = len(self.wave_zombies)
                ...
                return
                    ...
                    self.bar_type != c.CHOOSEBAR_STATIC and current_time - self.wave_time >= 23000)):
    ...
    def setupHugeWaveApprochingImage(self):
        ...
        self.huge_wave_approching_image = tool.get_image_alpha(tool.GFX[c.HUGE_WAVE_APPROCHING], *frame_rect, c.BLACK,
                                                               1)
    ...
    def setupLevelProgressBarImage(self):
        ...
        self.level_progress_zombie_head_image = tool.get_image_alpha(tool.GFX[c.LEVEL_PROGRESS_ZOMBIE_HEAD],
                                                                     *frame_rect, c.BLACK, 1)
    ...
    def equipmentEquipPlant(self, mouse_pos):
        """
        给植物添加装备liuxch
            ...
            if (x >= i.rect.x and x <= i.rect.right and
                ...
                if len(self.map.map[map_y][map_x][c.MAP_PLANT]) >= 2:
                    if c.LILYPAD in self.map.map[map_y][map_x][c.MAP_PLANT]:
                        ...
                        if i.name == "花盆（未实现）":
                            continue
                self.equipPlant(i, equipment=self.select_equipment)
                # 使用后默认装备清除
    ...
    def play(self, mouse_pos, mouse_click):
        ...
        if self.produce_sun:
            ...
            if (self.current_time - self.sun_timer) > min(c.PRODUCE_SUN_INTERVAL + 100 * self.fallen_sun,
                                                          9500) + random.randint(0, 2750):
                self.sun_timer = self.current_time
    ...
    def createZombie(self, name, map_y=None):
        ...
        match name:
            case c.NORMAL_ZOMBIE:
                self.zombie_groups[map_y].add(
                    zombie.NormalZombie(c.ZOMBIE_START_X + random.randint(-20, 20) + huge_wave_move, y,
                                        self.head_group))
            case c.CONEHEAD_ZOMBIE:
                self.zombie_groups[map_y].add(
                    zombie.ConeHeadZombie(c.ZOMBIE_START_X + random.randint(-20, 20) + huge_wave_move, y,
                                          self.head_group))
            case c.BUCKETHEAD_ZOMBIE:
                self.zombie_groups[map_y].add(
                    zombie.BucketHeadZombie(c.ZOMBIE_START_X + random.randint(-20, 20) + huge_wave_move, y,
                                            self.head_group))
            case c.FLAG_ZOMBIE:
                self.zombie_groups[map_y].add(zombie.FlagZombie(c.ZOMBIE_START_X, y, self.head_group))
            case c.NEWSPAPER_ZOMBIE:
                self.zombie_groups[map_y].add(
                    zombie.NewspaperZombie(c.ZOMBIE_START_X + random.randint(-20, 20) + huge_wave_move, y,
                                           self.head_group))
            case c.FOOTBALL_ZOMBIE:
                self.zombie_groups[map_y].add(
                    zombie.FootballZombie(c.ZOMBIE_START_X + random.randint(-20, 20) + huge_wave_move, y,
                                          self.head_group))
            case c.DUCKY_TUBE_ZOMBIE:
                self.zombie_groups[map_y].add(
                    zombie.DuckyTubeZombie(c.ZOMBIE_START_X + random.randint(-20, 20) + huge_wave_move, y,
                                           self.head_group))
            case c.CONEHEAD_DUCKY_TUBE_ZOMBIE:
                self.zombie_groups[map_y].add(
                    zombie.ConeHeadDuckyTubeZombie(c.ZOMBIE_START_X + random.randint(-20, 20) + huge_wave_move, y,
                                                   self.head_group))
            case c.BUCKETHEAD_DUCKY_TUBE_ZOMBIE:
                self.zombie_groups[map_y].add(
                    zombie.BucketHeadDuckyTubeZombie(c.ZOMBIE_START_X + random.randint(-20, 20) + huge_wave_move, y,
                                                     self.head_group))
            case c.SCREEN_DOOR_ZOMBIE:
                self.zombie_groups[map_y].add(
                    zombie.ScreenDoorZombie(c.ZOMBIE_START_X + random.randint(-20, 20) + huge_wave_move, y,
                                            self.head_group))
            case c.POLE_VAULTING_ZOMBIE:
                # 本来撑杆跳生成位置不同，对齐左端可认为修正了一部分（看作移动了70），只需要相对修改即可
                self.zombie_groups[map_y].add(
                    zombie.PoleVaultingZombie(c.ZOMBIE_START_X + random.randint(0, 10) + huge_wave_move, y,
                                              self.head_group))
            case c.ZOMBONI:
                # 冰车僵尸生成位置不同
                self.zombie_groups[map_y].add(
                    zombie.Zomboni(c.ZOMBIE_START_X + random.randint(0, 10) + huge_wave_move, y,
                                   self.plant_groups[map_y], self.map, plant.IceFrozenPlot))
            case c.SNORKELZOMBIE:
                # 潜水僵尸生成位置不同
                self.zombie_groups[map_y].add(
                    zombie.SnorkelZombie(c.ZOMBIE_START_X + random.randint(0, 10) + huge_wave_move, y, self.head_group))
    ...
    def addPlant(self):
        ...
        match self.plant_name:
            ...
            case c.COFFEEBEAN:
                new_plant = plant.CoffeeBean(x, y, self.plant_groups[map_y], self.map.map[map_y][map_x], self.map,
                                             map_x)
            case c.SEASHROOM:
    ...
    def setupMouseImage(self, item_name, selected_item, colorkey=c.BLACK):
        """
        liuxch
        :param item_name: 植物名称或装备名称
        :param selected_item: 所选物品
        :param colorkey:
        :return:
        """
        frame_list = tool.GFX[item_name]
        if item_name in c.PLANT_RECT:
            data = c.PLANT_RECT[item_name]
            x, y, width, height = data["x"], data["y"], data["width"], data["height"]
            self.mouse_image = tool.get_image(frame_list[0], x, y, width, height, colorkey, 1)
            self.mouse_rect = self.mouse_image.get_rect()
            self.drag_plant = True
            self.plant_name = item_name
            self.select_plant = selected_item

        elif item_name in c.EQUIPMENT_RECT:
            data = c.EQUIPMENT_RECT[item_name]
            x, y, width, height = data["x"], data["y"], data["width"], data["height"]
            self.mouse_image = tool.get_image(frame_list[0], x, y, width, height, colorkey, 1)
            self.mouse_rect = self.mouse_image.get_rect()
            self.drag_equipment = True
            self.equipment_name = item_name
            self.select_equipment = selected_item
    ...
    def removeMouseImage(self):
        self.drag_plant = False
        self.drag_equipment = False  # liuxch
                                ...
                                if bullet.name == c.BULLET_FIREBALL:
                                    for rangeZombie in self.zombie_groups[i]:
                                        if abs(rangeZombie.rect.x - bullet.rect.x) <= (c.GRID_X_SIZE // 2):
                                            rangeZombie.setDamage(c.BULLET_DAMAGE_FIREBALL_RANGE, effect=None,
    ...
    def checkZombieCollisions(self):
        for i in range(self.map_y_len):
            for zombie in self.zombie_groups[i]:
                        ...
                        if self.map.isValid(map_x, map_y):
                            if c.PUMPKINHEAD in self.map.map[map_y][map_x][c.MAP_PLANT]:
                                for actual_target_plant in self.plant_groups[i]:
                                    # 检测同一格的其他植物
                                    if self.map.getMapIndex(actual_target_plant.rect.centerx,
                                                            actual_target_plant.rect.bottom) == (map_x, map_y):
                ...
                if target_plant:
                    zombie.prey_map_x, zombie.prey_map_y = self.map.getMapIndex(target_plant.rect.centerx,
                ...
                if ((abs(zombie.rect.centerx - x) <= x_range) or
                        ((zombie.rect.right - (x - x_range) > 20) or (
                         ...
                         ((x + x_range) - zombie.rect.left > 20) or (
                                 (x + x_range) - zombie.rect.left) / zombie.rect.width > 0.2)[
                            zombie.rect.x > x]):  # 这代码不太好懂，后面是一个判断僵尸在左还是在右，前面是一个元组，[0]是在左边的情况，[1]是在右边的情况
        ...
        if not shovel:
            ...
            # 注意这里是在描述墓碑而非墓碑吞噬者
                ...
                if target_plant.name == c.DOOMSHROOM:
                    self.plant_groups[map_y].add(plant.Hole(target_plant.original_x, target_plant.original_y,
    ...
    def showLevelProgress(self, surface):
        ...
        # 按照当前波数生成僵尸头位置
        self.level_progress_zombie_head_image_rect.x = self.level_progress_bar_image_rect.x - int(
            (150 * self.wave_num) / (self.map_data[c.NUM_FLAGS] * 10)) + 145  # 常数为拟合值
        self.level_progress_zombie_head_image_rect.y = self.level_progress_bar_image_rect.y - 3  # 常数为拟合值

        # 填充的进度条信息
        # 常数为拟合值
        filled_bar_rect = (self.level_progress_zombie_head_image_rect.x + 3, self.level_progress_bar_image_rect.y + 6,
                           int((150 * self.wave_num) / (self.map_data[c.NUM_FLAGS] * 10)) + 5, 9)
        # 画填充的进度条
        pg.draw.rect(surface, c.YELLOWGREEN, filled_bar_rect)

        # 画旗帜
        for i in range(self.num_flags):
            self.level_progress_flag_rect.x = self.level_progress_bar_image_rect.x + int(
                (150 * i) / self.num_flags) + 5  # 常数是猜的
            # 当指示进度的僵尸头在旗帜左侧时升高旗帜
